...
type RootStackParamList = {
  SearchFilter: {
    currentFilters?: any;
    myGender?: string | number;
    onApply?: (filters: any) => void;
    regionName?: string;
  };
};

type SearchFilterScreenRouteProp = RouteProp<RootStackParamList, 'SearchFilter'>;
type SearchFilterScreenNavigationProp = StackNavigationProp<RootStackParamList, 'SearchFilter'>;

type Props = {
  route: SearchFilterScreenRouteProp;
...
const PrefectureModal = ({ visible, selectedValues, onSelect, onClose }: PrefectureModalProps) => {
  ...
  const toggleSelection = (pref: string) => {
    let newSelected;
...
export default function SearchFilterScreen({ navigation, route }: Props) {
  ...
  const [maxAge, setMaxAge] = useState<number>(initialFilters.maxAge || 40);
  const [enableHeightFilter, setEnableHeightFilter] = useState<boolean>(initialFilters.enableHeightFilter || false);
  const [minHeight, setMinHeight] = useState<number>(initialFilters.minHeight || 150);
  ...
  const [education, setEducation] = useState<string[]>(initialFilters.education || []);
  const [income, setIncome] = useState<string[]>(initialFilters.income || []);
  const [holiday, setHoliday] = useState<string[]>(initialFilters.holiday || []);
  const [alcohol, setAlcohol] = useState<string[]>(initialFilters.alcohol || []);
  const [tobacco, setTobacco] = useState<string[]>(initialFilters.tobacco || []);
  const [roommate, setRoommate] = useState<string[]>(initialFilters.roommate || []);
  const [bloodType, setBloodType] = useState<string[]>(initialFilters.bloodType || []);
  const [marry, setMarry] = useState<string[]>(initialFilters.marry || []);
  const [date, setDate] = useState<string[]>(initialFilters.date || []);
  const [child, setChild] = useState<string[]>(initialFilters.child || []);
  const [marital, setMarital] = useState<string[]>(initialFilters.marital || []);
  const [sibling, setSibling] = useState<string[]>(initialFilters.sibling || []);
  const [encounter, setEncounter] = useState<string[]>(initialFilters.encounter || []);
  const [occupation, setOccupation] = useState<string[]>(initialFilters.occupation || []);
  const [personality, setPersonality] = useState<string[]>(initialFilters.personality || []);
  const [workTime, setWorkTime] = useState<string[]>(initialFilters.workTime || []);
  const [lifeStyle, setLifeStyle] = useState<string[]>(initialFilters.lifeStyle || []);
  const [contactFrequency, setContactFrequency] = useState<string[]>(initialFilters.contactFrequency || []);
  const [cookingFrequency, setCookingFrequency] = useState<string[]>(initialFilters.cookingFrequency || []);
  const [birthPlace, setBirthPlace] = useState<string[]>(initialFilters.birthPlace || []);
  const [regionName, setRegionName] = useState<string>(initialFilters.regionName || '未設定');
  ...
  const [birthPlaceModalVisible, setBirthPlaceModalVisible] = useState<boolean>(false);
