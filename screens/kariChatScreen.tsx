...
export default function ChatScreen({ navigation, route }: Props) {
  const { matchId, recipientId, recipientName, recipientImage } =
    route.params || {};
  ...
  const [recipientUnreadCount, setRecipientUnreadCount] = useState<number>(-1);
  ...
  const readStatusMap = useMemo(() => {
    ...
    if (recipientUnreadCount === -1) return map;

    let unreadCountRemaining = recipientUnreadCount;

    for (let i = messages.length - 1; i >= 0; i--) {
      const msg = messages[i];
      if (msg.senderId === currentUser?.uid) {
        if (unreadCountRemaining > 0) {
          map.set(msg.id, false);
          unreadCountRemaining--;
        } else {
          map.set(msg.id, true);
        }
      }
    ...
    return map;
  }, [messages, recipientUnreadCount, currentUser?.uid]);
    ...
    const initChat = async () => {
      ...
      try {
        ...
        if (matchSnap.exists()) {
          ...
          if (matchData) {
            const deletedAtVal =
              matchData.deletedAt && matchData.deletedAt[currentUser.uid];
            if (deletedAtVal) {
              myDeletedAtMs = deletedAtVal.toDate
                ? deletedAtVal.toDate().getTime()
                : new Date(deletedAtVal).getTime();
            }
          }
        }
      } catch (error) {
      ...
      unsubscribeMatch = onSnapshot(matchRef, (docSnap) => {
        if (docSnap.exists()) {
          ...
          if (matchData && matchData.unreadCounts) {
            setRecipientUnreadCount(matchData.unreadCounts[recipientId] || 0);
          }
        }
      });
      ...
      unsubscribeMessages = onSnapshot(
        ...
        (snapshot) => {
          ...
          snapshot.docs.map((docData: QueryDocumentSnapshot<DocumentData>) => {
            const data = docData.data();
            const createdAtDate = data.createdAt
              ? data.createdAt.toDate()
              : new Date();

            if (createdAtDate.getTime() >= myDeletedAtMs) {
    ...
    try {
      ...
      if (isLatesMessage) {
        ...
        if (updateMessages.length > 0) {
          ...
          await updateDoc(matchRef, {
            lastMessage: previewText,
            lastMessageAt: newLatesMsg.createdAt,
          });
        } else {
          await updateDoc(matchRef, {
  ...
  const handleAvatarPress = async () => {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        let defaultImage = require("../assets/man.png");
        const gender = rawData.gender;
        if (gender === "女性" || gender === "female" || gender === 2) {
          defaultImage = require("../assets/woman.png");
        }
      ...
      <View>
        ...
        >
          ...
          >
            ...
            >
              {isMe && readStatusMap.get(item.id) && (
